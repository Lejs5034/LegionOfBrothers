...
export function getEffectiveRole(
  ...
  const globalRankInfo = getRankDisplayInfo(globalRank);
  ...
  if (serverRole.role_key && RANK_LABELS[serverRole.role_key]) {
    const roleInfo = RANK_LABELS[serverRole.role_key];
    return {
  ...
  const roleKeyMatch = Object.entries(RANK_LABELS).find(
    ([_, info]) => info.label.toLowerCase().trim() === normalizedServerLabel
  );

  if (roleKeyMatch) {
    const [_, roleInfo] = roleKeyMatch;
    return {
      label: roleInfo.label,
      emoji: roleInfo.emoji,
      color: roleInfo.color,
  ...
  const globalRankKeys = ['the_head', 'app_developer', 'admin'];
  if (globalRankKeys.includes(globalRank)) {
    return {
      label: globalRankInfo.label,
      emoji: globalRankInfo.emoji,
      color: globalRankInfo.color,
